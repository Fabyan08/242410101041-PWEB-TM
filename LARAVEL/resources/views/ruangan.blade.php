    ...
    <div class="bg-slate-50 dark:bg-slate-900 min-h-screen pb-20 transition-colors duration-300">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if (Auth::check() == false)
                ...
                <div id="ruangan-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    @empty
                        <div
                            ...
                            <h3 class="text-lg font-bold text-slate-700 dark:text-slate-200">Ruangan tidak ditemukan
                            </h3>
                            <p class="text-slate-500 dark:text-slate-400 mt-1">Coba gunakan kata kunci atau kategori
                                lain.
                            </p>
                            <a href="{{ route('ruangan.index') }}" id="btn-reset-search"
                                class="mt-4 inline-block text-orange-500 font-semibold hover:underline">Reset
                                Pencarian</a>
                        </div>
                    @endforelse
                </div>
                ...
                @if ($ruangans->hasPages())
                    <div id="pagination-wrapper" class="mt-12 flex justify-center relative z-20 pointer-events-auto">
                        {{ $ruangans->links() }}
                    </div>
                @else
            ...
            @auth
                @if (auth()->user()->status === 'Suspended')
                    <div
                        ...
                        <div class="ml-4">
                            <h3 class="text-sm font-bold text-red-800 dark:text-red-400 tracking-wide uppercase">Akses
                                Pemesanan
                                Diblokir</h3>
                            <p class="text-sm text-red-600 dark:text-red-300 mt-1 leading-relaxed">
                                Akun Anda saat ini sedang dalam status ditangguhkan. Anda masih dapat menjelajahi katalog,
                                namun
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const gridContainer = document.getElementById('ruangan-grid');
        ...
        function fetchData(url) {
            ...
            fetch(url, {
                ...
                .then(html => {
                    ...
                    const newGrid = doc.getElementById('ruangan-grid');
        ...
        document.addEventListener('click', function(e) {
            if (e.target && e.target.id === 'btn-reset-search') {
                e.preventDefault();
                searchInput.value = '';
                const resetUrl = e.target.getAttribute(
                    'href');
                fetchData(resetUrl);
            }
        });
