...
const updateStats = () => {
  document.getElementById("stat-total-booking").innerText = bookingData.length;
...
const renderTable = () => {
  ...
  const filteredData = bookingData.filter((item) => {
    ...
    const matchFilter = filterTerm === "All" || item.room.includes(filterTerm);
...
form.addEventListener("submit", (e) => {
  ...
  if (inputs.editId.value) {
    const index = bookingData.findIndex((b) => b.id === inputs.editId.value);
    if (index !== -1) {
